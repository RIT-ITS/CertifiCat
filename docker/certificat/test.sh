...
docker run --network docker_default -e CERTIFICAT_CONFIG=/srv/www/config.yml -v"${WORKSPACE}/docker/certificat/srv/www/entrypoint.sh:/srv/www/entrypoint.sh" -v"${CONFIG_FILE}:/srv/www/config.yml" --entrypoint=/bin/bash -p8000:80 -it $LAST_IMAGE
